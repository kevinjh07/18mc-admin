...
export class EventRegistrationComponent implements OnInit {
  ...
  get(id: number) {
    this.eventService.get(id).subscribe({
      ...
      error: (e) => {
        this.logger.error(e);
        this.blockUI.stop();
        this.notificationService.openSnackBar("Erro ao obter dados do evento, tente novamente");
      },
    });
  }
  ...
  save() {
    ...
    this.eventService.save(this.event).subscribe({
      next: () => {
        ...
        this.notificationService.openSnackBar("Evento cadastrado com sucesso!");
      },
      error: (e) => {
        this.blockUI.stop();
        this.logger.error(e);
        this.notificationService.openSnackBar("Erro ao salvar evento, tente novamente");
      },
    });
  }
  ...
  update() {
    this.blockUI.start("Aguarde...");
    this.event.personIds = this.selectedPerson.map((sp) => sp["id"]);
    this.eventService.update(this.event).subscribe({
      next: () => {
        this.blockUI.stop();
        this.router.navigate(["/event"]);
        this.notificationService.openSnackBar("Evento atualizado com sucesso!");
      },
      error: (e) => {
        this.blockUI.stop();
        this.logger.error(e);
        this.notificationService.openSnackBar("Erro ao atualizar evento, tente novamente");
      },
    });
  }
  ...
  getPersons() {
    this.blockUI.start("Aguarde...");
    this.personService.getAllByDivision(this.event.divisionId).subscribe({
      next: (response: any) => {
        this.blockUI.stop();
