...
export class PollListComponent implements OnInit {
  ...
  ngAfterViewInit(): void {
    ...
    if (filter.pollListCommandId) {
      this.selectedCommandId = filter.pollListCommandId;
      if (filter.pollListRegionalId) {
        this.selectedRegionalId = filter.pollListRegionalId;
        this.getDivisions();
        if (filter.pollListDivisionId) {
          this.selectedDivisionId = filter.pollListDivisionId;
        }
        this.getPolls();
      }
    }
  ...
  getPolls() {
    ...
    this.eventService
      .getAll(this.paginator.pageIndex + 1, this.paginator.pageSize, EventType.POLL, this.selectedRegionalId, this.selectedDivisionId)
      .subscribe({
        next: (response: any) => {
          this.blockUI.stop();
        ...
        error: (e) => {
          this.blockUI.stop();
          this.logger.error(e);
          this.notificationService.openSnackBar("Erro ao carregar lista de enquetes");
        },
      });
  }
  ...
  getRegionals() {
    ...
    this.filterService.updateFilter({ pollListCommandId: this.selectedCommandId });
  ...
  getDivisions() {
    this.filterService.updateFilter({ pollListRegionalId: this.selectedRegionalId });
  ...
  showParticipantsDialog(id: number) {
    this.dialog.open(PollParticipantDialogComponent, {
      width: "80%",
      data: {
        pollId: id,
      },
